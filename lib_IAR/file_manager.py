    ...
    def publish_firmware(self, source_bin_path: str, commit_id: str, version: str,
        ...
        try:
            ...
            if self.copy_file(source_bin_path, destination_path):
                ...
                # 如果需要发布.out文件
                if publish_out_file:
                    logger.info(f"需要发布.out文件，publish_out_file: {publish_out_file}")
                    out_file_path = None
                    if self.path_manager:
                        logger.info("path_manager存在，开始查找.out文件")
                        out_file_path = self.path_manager.find_out_file(configuration=configuration)
                    else:
                        logger.warning("path_manager为None，无法查找.out文件")
                    ...
                    if out_file_path and os.path.exists(out_file_path):
                        ...
                        if self.copy_file(out_file_path, out_destination_path):
                            ...
                            logger.info(f".out文件发布成功: {out_file_path} -> {out_destination_path}")
                        else:
                            success_msg += f"\n.out文件发布失败: {out_file_path}"
                            logger.warning(f".out文件发布失败: {out_file_path}")
                    else:
                        success_msg += f"\n未找到对应的.out文件"
                        logger.warning(f"未找到对应的.out文件，源bin文件: {source_bin_path}")
            ...
            if publish_out_file:
                ...
                if out_file_path and os.path.exists(out_file_path):
                    ...
                    if self.copy_file(out_file_path, remote_out_path):
                        ...
                        logger.info(f".out文件已复制到远程目录: {remote_out_path}")
                    else:
                        logger.warning(f"复制.out文件失败: {out_file_path}")
                else:
                    logger.warning(f"未找到对应的.out文件，源bin文件: {bin_file_path}")
...
def test_iar_file_manager():
    ...
    manager = IARFileManager(config, project_path="E:/TestProject")
